Component({
  ...
  observers: {
    ...
    var size = this.properties.fileinfo.file_size
    if (size < 1024 * 1024) {
      that.setData({
    ...
    var name = this.properties.fileinfo.file_name
  ...
  methods: {
    go() {
      wx.navigateTo({
        url: '/pages/filedetail/main?id=' + this.properties.fileinfo._id + '&fileimg=' + this.data.fileimg,
        fail: (e) => {
          console.log(e);
        }
      })
